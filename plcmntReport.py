...
import dash  									# Plotly web Framework for deploying graphs
import dash_core_components as dcc 
import dash_html_components as html
from dash.dependencies import Input, Output
import base64 									# Package required for encoding image 
...
fig1.update_layout(
	# width = 800,
	 height = 600,
	 title = {'text':'Fig1 - Summary Table', 'x': 0.48, 'y': 0.88},
...
fig5.update_layout(barmode = 'stack', showlegend = False, 
					...
					font = dict(size = 20), template = 'presentation', height = 600, width = 1400)
# fig5.show()
...
fig6A.update_layout(title = {'text': 'Fig7 - Package\'s Inequality', 'y':0.91}, font = dict(size = 18),
...
fig6B.update_layout(template = 'presentation', title = {'text':'Fig6 - Statistical Summaries of Salary Package'},
...
# Dash App
external_stylesheets = ['https://codepen.io/chriddyp/pen/bWLwgP.css']	# The sheet is a css file containing info about asthetics of the page layout
app = dash.Dash(__name__, external_stylesheets = external_stylesheets)	# Declaring app

# Calling image from local and encoding it
img_fn = '/home/inblr02-linux-07/Desktop/Documents/Programming_Workspaces/git_repos/data_visualisation/plcmnt.jpg'
encoded_img = base64.b64encode(open(img_fn, 'rb').read())

# The text to be displayed is written in markdown format and whole text is divided into blocks as it was needed to place them seperately in report 
markdown_text_1 = '''
The report analyse the placment statistics provided by *Ben Roshan D* under heading [Campus Recruitment](https://www.kaggle.com/benroshan/factors-affecting-campus-placement).
The primary objective is to discover patterns as to how different factors affect the placement of a candidate both in terms of chances of his/her being getting placed and the
salary package obtained by a candidate if placed. The data is being analysed using ***Exploratory Data Analysis*** approach. There are two aspects of data that we try to
capture, the first being the properties of the data such as it's composition and usability, and the second aspect is to capture the statistical properties of data and how 
...
'''
markdown_text_4 = '''
It can be seen from figure 2 that only salary column have missing value but this is because for candidates who didn't get placed in campus placement or opted out of it, there salary
...
distribution of salary i.e. how evenly it is distributed. For this we look at figure 6 and figure 7. Figure 6 shows the distribution of salary and figure 7 shows the inequality in package
...
# Defining app (report) layout.
app.layout = html.Div(children = [
				html.Img(src = 'data:image/jpg;base64, {}'.format(encoded_img.decode()), height = 300, width = 1730),	# Adding Image

				html.H1(children = 'Analysis of MBA Placements at XYZ University: EDA Approach',						# Adding Heading
					    style = {'fontSize':50, 'fontWeight':25, 'textAlign':'center', 'color':'black', 'marginTop': 20}),

				html.Div(children = '''Ayush Bansal''', style = {'textAlign':'center', 'fontSize':30, 'fontWeight':15,	# Adding Name
							'color':'black'}),

				html.Div(children = '''Data Scientist, Maersk GSC''', style = {'textAlign':'center', 'fontSize':30, 
							'fontWeight':15, 'color':'black', 'marginTop': -8}),

				html.Div(children = '''Introduction''', style = {'fontSize':40, 'fontWeight':15, 'color':'black',
							'marginLeft':30, 'marginTop':60, 'textDecoration':'underline'}),

				html.Div(children = [dcc.Markdown(children = markdown_text_1)], style = {'fontSize':25, 'marginTop':-10,
						 'marginLeft':30}), 

				html.Div(children = '''Analysis and Interpretations''', style = {'fontSize':40, 'fontWeight':15, 'color':'black',
						 'marginLeft':30, 'marginTop':20, 'textDecoration':'underline'}),

				html.Div(children = [dcc.Markdown(children = markdown_text_2)], style = {'fontSize':25, 'marginTop':-10,
						 'marginLeft':30}),

				html.Div(dcc.Graph(id = 'Summary Table',figure = fig1),style = {'marginTop':-5, 'width':'70%', 'marginLeft':250}),

				html.Div(children = dcc.Markdown(children = markdown_text_3), style = {'fontSize':25, 'marginTop':-10,
						 'marginLeft':30}),

				html.Div(children = [dcc.Graph(id = 'Missing Rows Bar Chart', figure = fig2)],
						  style = {'width':'59%', 'display':'inline-block'}),

				html.Div(children = [dcc.Graph(id = 'Variable Composition Pie Chart', figure = fig3)],
						  style = {'width':'39%', 'float':'right', 'display':'inline-block'}),

				html.Div(children = dcc.Markdown(children = markdown_text_4), style = {'fontSize':25, 'marginTop':20,
						  'marginLeft':30}),

				html.Div(children = dcc.Graph(id = 'Boxplot of Statistical Summaries', figure = fig4),
						  style = {'marginLeft':30}),

				html.Div(children = dcc.Markdown(children = markdown_text_5), style = {'fontSize':25, 'marginTop':20,
						 'marginLeft':30}),

				html.Div(children = dcc.Graph(id = 'Stacked Bar Chart Categorical Data Composition', figure = fig5),
						  style = {'marginLeft':140}),

				html.Div(children = dcc.Markdown(children = markdown_text_6), style = {'fontSize':25, 'marginTop':20,
						  'marginLeft':30}),

				html.Div(children = dcc.Graph(id = 'Salary Distribution Histogram', figure = fig6B),
						 style = {'width':'60%', 'display':'inline-block'}),

				html.Div(children = dcc.Graph(id = 'Salary inequality Curve', figure = fig6A),
						 style = {'width':'40%', 'display':'inline-block', 'float':'right'}),

				html.Div(children = dcc.Markdown(children = markdown_text_7), style = {'fontSize':25, 'marginTop':20,
						 'marginLeft':30}),

				html.Div(children = dcc.Graph(id = 'Correlation Heatmap', figure = fig7), style = {'marginLeft':140}),

				html.Div(children = dcc.Markdown(children = markdown_text_8), style = {'fontSize':25, 'marginTop':20,
						  'marginLeft':30})
	])


if __name__ == '__main__':				# Running the app on loop
	app.run_server(debug = False, port = 8050)	# define arguements port at which is being run and debug mode

# Note: Rather than running app from python console run it from terminal, so that you can switch it off using Ctrl+C.
# In python console Ctrl+C, does not work for me. Also if you have already deployed the app once and when you run it
# again you get error that 'Already in use', then change port value in order to deploy app at different terminal.
# Also if you want to make changes in app layout, then keep debug mode on. While the app is running, if you make any
# changes in code and then save it (Ctrl+S), then dash will automatically make changes to the deployed app.
